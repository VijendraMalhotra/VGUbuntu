...
DEST = LIBDIR/'vmware-installer/3.1.0'
...
class VMwareInstaller(Installer):
   ...
   def PreTransactionUninstall(self, old, new, upgrade):
      ...
      if askQuestion:
         ...
         if currentVersion and \
            Version(currentVersion) == Version('3.1.0') and \
            not upgrade:
   ...
   def PostInstall(self, old, new, upgrade):
      ...
      self.SetConfig('%s.vmisloc' % '3.1.0', DEST)
      self.SetConfig('%s.pyloc'   % '3.1.0', DEST/'python')
      self.SetConfig('%s.pyver'   % '3.1.0', PYTHON_VERSION)
      ...
      if systemVersion is None or Version(systemVersion) <= Version(new):
         ...
         self.SetConfig('currentVersion', '3.1.0')
   ...
   def PreUninstall(self, old, new, upgrade):
      ...
      self.DelConfig('%s.vmisloc' % '3.1.0')
      self.DelConfig('%s.pyloc'   % '3.1.0')
      self.DelConfig('%s.pyver'   % '3.1.0')
   ...
   def PostUninstall(self, old, new, upgrade):
      ...
      if currentVersion and Version(currentVersion) == Version('3.1.0'):
         if keepConfig != 'yes':
